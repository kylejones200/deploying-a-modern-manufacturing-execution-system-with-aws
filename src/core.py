...
def analyze_manufacturing_data(df: pd.DataFrame, timestamp_col: str, production_col: str) -> pd.DataFrame:
...
def calculate_production_metrics(df: pd.DataFrame, production_col: str) -> dict:
    ...
    return {'total_production': df[production_col].sum(), 'mean_production': df[production_col].mean(), 'std_production': df[production_col].std(), 'efficiency': df[production_col].mean() / df[production_col].max() if df[production_col].max() > 0 else 0}
