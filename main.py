...
def main():
    parser = argparse.ArgumentParser(
        description="Deploying Manufacturing Execution System with AWS"
    )
    ...
    if args.data_path and args.data_path.exists():
        ...
        df = analyze_manufacturing_data(
            df, config["data"]["timestamp_column"], config["data"]["production_column"]
        )
        ...
        production = (
            100
            + 20 * np.sin(np.arange(config["data"]["n_periods"]) / 24)
            + np.random.normal(0, 5, config["data"]["n_periods"])
        )
